let lengths = [106,16,254,226,55,2,1,166,177,247,93,0,255,228,60,36];
...
initializeList(list);
sparseHash(lengths, list, infoObj);
...
function knotHash(lengths, list, infoObj) {
	lengths.forEach(length => {
		...
		infoObj.currPosition = (infoObj.currPosition + length + infoObj.skip) % list.length;
...
function sparseHash(lengths, list, infoObj){
	let ascArr = lengths.join().split("").map((val) => val.charCodeAt(0));
